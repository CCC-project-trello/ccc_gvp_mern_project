...
const Board = () => {
  ...
  const onRemoveTask = (taskId) => {
    ...
    // Loop through all columns and remove the task by ID
    for (const columnId in updatedColumns) {
      updatedColumns[columnId].items = updatedColumns[columnId].items.filter(task => task.id !== taskId);
    }
    setColumns(updatedColumns); // Update state with the removed task
  };
        ...
        <div className="board">
          {Object.entries(columns).map(([columnId, column]) => (
            <Column
              ...
              columnId={columnId}
              column={column}
              onRemoveTask={onRemoveTask} // Pass onRemoveTask function to Column
            />
          ))}
        </div>
      </DragDropContext>
